 ...
 * React version of admin-panel.html, structured to mirror
 * Community.jsx (Screen 10) / ItineraryView.jsx (Screen 9)
 * for 1:1 consistency.
 *
 * Usage:
 ...
 *
 * Requires admin-panel.css in the same folder.
 */
  ...
  return (
    <div onClick={() => menuOpen && setMenuOpen(false)}>
      ...
      <div className="page-head">
        <div className="wrap">
          <span className="eyebrow">Screen 12</span>
          <h1>Admin panel</h1>
        ...
        <div className="toolbar">
          <div className="search-field">
            ...
            <input
              ...
              value={search}
              disabled={!usesToolbar}
              onChange={(e) => setSearch(e.target.value)}
            />
          </div>
          <div className="toolbar-selects">
            <select aria-label="Group by" value={groupBy} disabled={tab !== 'users'} onChange={(e) => setGroupBy(e.target.value)}>
              <option value="none">Group by: None</option>
              <option value="status">Group by: Status</option>
            </select>
            <select aria-label="Filter" value={filter} disabled={tab !== 'users'} onChange={(e) => setFilter(e.target.value)}>
            ...
            <select aria-label="Sort by" value={sortBy} disabled={!usesToolbar} onChange={(e) => setSortBy(e.target.value)}>
        ...
        {tab === 'users' && (
          <section>
            <table className="data-table">
              ...
              <tbody>
                {visibleUsers.map((u) => (
                  <tr key={u.id} onClick={() => console.log(`Opened user "${u.name}"`)}>
                    <td>
        ...
        {tab === 'cities' && (
          <section className="rank-board">
            {visibleCities.map((c, i) => (
              <div className="rank-board-row" key={c.id} onClick={() => console.log(`Opened "${c.name}"`)}>
        ...
        {tab === 'activities' && (
          <section className="rank-board">
            {visibleActivities.map((a, i) => (
              <div className="rank-board-row" key={a.id} onClick={() => console.log(`Opened "${a.name}"`)}>
                <div className="rank-board-num">{i + 1}</div>
                <div className="rank-board-main">
